...
router.get('/irelandstudytour', (req, res, next) => {
  ...
  request.get('https://api.shingo.org/salesforce/events/a1B1200000Rin6u')
  .then(results => {
    ...
    // Get Session Map
    return request.get('https://api.shingo.org/salesforce/events/sessions?event_id=a1B1200000Rin6u')
  })
  .then(results => {
    ...
    // Get list of Days
    return request.get('https://api.shingo.org/salesforce/events/days?event_id=a1B1200000Rin6u')
  })
    .then(results =>{
        const response = JSON.parse(results)
    ...
    .catch(err =>{
        console.error("IRELAND STUDY TOUR\n", err)
    res.render('education/irelandstudytour', {
      event: event_info,
    })
  })
